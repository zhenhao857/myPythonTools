...
def get_and_set_construction_mileage_value(row_value, target_xls_sheet):
    ...
    work_part_value = "DK"+str(work_part_value)[0:3]+"+"+str(work_part_value)[3:len(str(work_part_value))]+"第④部"
...
def get_and_set_max_overbreak_value(row_value, target_xls_sheet):
    ...
    target_xls_sheet.write(14, 1, work_part_value_A if work_part_value_A else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(15, 1, work_part_value_B if work_part_value_B else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(16, 1, work_part_value_C if work_part_value_C else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(17, 1, work_part_value_D if work_part_value_D else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(18, 1, work_part_value_E if work_part_value_E else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(19, 1, work_part_value_F if work_part_value_F else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(20, 1, work_part_value_G if work_part_value_G else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(21, 1, work_part_value if work_part_value else '/', set_style('宋体', 200, False))
...
def get_and_set_actual_overbreak_value(row_value, target_xls_sheet):
    ...
    target_xls_sheet.write(14, 5, work_part_value_A if work_part_value_A else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(15, 5, work_part_value_B if work_part_value_B else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(16, 5, work_part_value_C if work_part_value_C else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(17, 5, work_part_value_D if work_part_value_D else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(18, 5, work_part_value_E if work_part_value_E else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(19, 5, work_part_value_F if work_part_value_F else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(20, 5, work_part_value_G if work_part_value_G else '/', set_style('宋体', 200, False))
    target_xls_sheet.write(21, 5, work_part_value if work_part_value else '/', set_style('宋体', 200, False))
...
def covert_sheet1_taijie(sheet_0, file_target_sheet_num, file_target_xls_cp, log_txt):
    ...
    if covert_row_num > file_target_sheet_num:
        ...
        for sheet_num in range(begin, covert_row_num):
            ...
            # 设置目标sheet名称
            target_xls_sheet.name = str(row_num+1)  # 如果是数值类型则报错
...
def convert_excel(origin_xls, target_xls, log_file, save_path_name):
    ...
    name = file_origin_xls_sheets[2].name
    if name == '4部':
        print("正在写入"+name+"sheet" + origin_xls)
        print("正在写入"+name+"sheet" + origin_xls, file=log_txt)
        covert_sheet1_taijie(file_origin_xls_sheets[2], len(file_target_xls.sheets()), file_target_xls_cp, log_txt)
...
def start():
    ...
    # 目标excel
    target_xls = "C:\\mengxiaoqing\\20200614\\4部-模板.xls"
    log_file = "C:\\mengxiaoqing\\20200614\\4部日志.txt"
    # 保存的文件名
    save_path_name = "C:\\mengxiaoqing\\20200614\\4部结果.xls"
